...
/**
 * @ngdoc directive
 * @name wehrApp.directive:jqGridDirective
 * @description
 * # jqGridDirective
 */
angular.module('wehrApp')
  .directive('jqGridDirective', function () {
    return {
    	restrict: 'E', 
    	replace: true, 
    	scope: { 
    			...
    			scope.api = {
    					insert: function (rows) {
    						if (rows) {
    							...
    							table.jqGrid('setGridParam', { data: scope.modeldata }).trigger('reloadGrid'); 
    	                    } 
    					},clear: function () {
    	                	scope.modeldata.length = 0;
    	                	table.jqGrid('clearGridData', { data: scope.modeldata }).trigger('reloadGrid');
    	                },refresh: function () {
    	                	table.jqGrid('clearGridData').jqGrid('setGridParam', { data: scope.modeldata }).trigger('reloadGrid');
    	                }
    			}; 
    		}); 
